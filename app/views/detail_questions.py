...
from app.models import DetailQuestion, Answer  # 데이터베이스의 DetailQuestion 및 Answer 테이블과 매핑된 모델 클래스.

# Flask에서 기능을 모듈화하기 위해 Blueprint를 사용한다.
# "detail_questions"라는 이름의 Blueprint를 생성하고, /detail-questions 경로로 연결한다.
detail_questions_dp = Blueprint("detail_questions", __name__, url_prefix="/detail-questions")

# 모든 상세 질문 조회
@detail_questions_dp.route("/", methods=["GET"])  # GET 요청에 반응하는 엔드포인트를 정의한다.
def get_detail_questions():  

    # 데이터베이스에서 DetailQuestion 테이블의 모든 레코드를 조회한다.
...
# 특정 상세 질문 조회
@detail_questions_dp.route("/<int:detail_questions_id>", methods=["GET"])  # URL 경로에 숫자형 ID를 포함해 요청을 처리한다.
def get_detail_question(detail_questions_id):  

    # 데이터베이스에서 입력받은 ID에 해당하는 레코드를 검색한다.
    detail_question = DetailQuestion.query.get(detail_questions_id)  
...
# 특정 조건에 맞는 상세 질문 조회
@detail_questions_dp.route("/filtered", methods=["GET"])  # GET 요청에 반응하는 새로운 엔드포인트를 정의한다.
def get_filtered_detail_questions():

    # 데이터베이스에서 조건(sqe: 1~4, is_active: True)에 맞는 레코드를 필터링한다.
    filtered_questions = DetailQuestion.query.filter(
        DetailQuestion.sqe.between(1, 4),  # sqe가 1과 4 사이인 레코드
        DetailQuestion.is_active == True  # 활성화된 레코드만 포함
    ).order_by(DetailQuestion.sqe).all()  # sqe 기준으로 정렬

    # 조회된 데이터를 JSON 형식으로 반환한다.
    return jsonify([dq.to_dict() for dq in filtered_questions]), 200

# 새로운 상세 질문 생성
@detail_questions_dp.route("/", methods=["POST"])  # POST 요청을 처리하는 엔드포인트를 정의한다.
