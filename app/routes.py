...
def results(user_id):
    ...
    # 모든 MBTI 비율
    mbti_count = {}
    for mbti_type in ['INTJ','INTP','ENTJ','ENTP','INFJ','INFP','ENFJ','ENFP','ISTJ','ISFJ','ESTJ','ESFJ','ISTP','ISFP','ESTP','ESFP']:  # 여기에 실제 MBTI 타입을 다 넣으세요.
    ...
    for idx, question in enumerate(questions):
        ...
        # 차트 생성
        chart_colors = [custom_colors[i % len(custom_colors)] for i in range(len(choice_counts))]
        chart = px.pie(
            names=list(choice_counts.keys()),
            values=list(choice_counts.values()),
            title=f"Question {question.sqe}: {question.title}"
        )
        chart.update_traces(marker=dict(colors=chart_colors))  # 색상 설정
